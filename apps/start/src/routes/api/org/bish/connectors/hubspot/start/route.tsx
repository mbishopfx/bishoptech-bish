...
export const Route = createFileRoute('/api/org/bish/connectors/hubspot/start')({
  server: {
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          if (!connectorAccountId) {
            return Response.redirect(
              new URL(
                '/organization/settings/connectors?connectorAuth=error&provider=hubspot&message=Missing%20connector%20account',
                request.url,
              ),
              302,
            )
          ...
          if (!result.redirectUrl) {
            return Response.redirect(
              new URL(
                '/organization/settings/connectors?connectorAuth=success&provider=hubspot',
                request.url,
              ),
              302,
            )
          ...
          return Response.redirect(
            new URL(
              `/organization/settings/connectors?connectorAuth=error&provider=hubspot&message=${message}`,
              request.url,
            ),
            302,
          )
